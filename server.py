...
DB_FILE = os.path.join(BASE_DIR, "database_keys.json")
...
def api_check_device():
    # force=True: parse JSON regardless of Content-Type header
    # This fixes the "Missing device_id" error when client omits Content-Type: application/json
    data = request.get_json(silent=True, force=True) or {}
    device_id = (
        data.get('device_id', '') or data.get('hwid', '') or data.get('id', '') or
        request.form.get('device_id', '') or request.form.get('hwid', '') or
        request.args.get('device_id', '') or request.args.get('hwid', '')
    ).strip()
    if not device_id:
        return jsonify({"status": "error", "message": "Missing device_id"})
    db = load_db()
...
@app.route('/api/submit_device_request', methods=['POST'])
...
@app.route('/api/list_device_requests', methods=['GET'])
...
@app.route('/api/approve_device_request', methods=['POST'])
...
@app.route('/api/check_device_approval', methods=['POST'])
...
@app.route('/api/check-and-request-device', methods=['POST'])
def check_and_request_device():
    """
    Combined endpoint used by the Settings tab device checker.
    1. Accepts device_id via JSON (with or without Content-Type header), form data, or query params.
    2. If approved and valid  → returns status=approved + time info.
    3. If not found or expired → auto-submits an approval request (if not already pending)
       and returns status=pending so the UI can show "waiting for admin".
    """
    data = request.get_json(silent=True, force=True) or {}
    device_id = (
        data.get('device_id', '') or data.get('hwid', '') or data.get('id', '') or
    ...
    # --- Auto-submit a pending request for admin approval ---
    requests_map = db.get("___DEVICE_REQUESTS___", {})
    for rid, rinfo in requests_map.items():
        if rinfo.get('device_id') == device_id and rinfo.get('status') == 'pending':
            return jsonify({
                ...
                "req_id": rid,
                "submitted_at": format_ts(rinfo.get('submitted_at', 0))
            })

    # Create a new pending request automatically
    req_id = str(int(now * 1000)) + "-" + "".join(random.choices(string.ascii_uppercase + string.digits, k=4))
    requests_map[req_id] = {
        "device_id": device_id,
        "val": "7",
        "unit": "ngày",
        "note": "Tự động gửi từ Settings",
        "status": "pending",
        "submitted_at": now,
        "ip": get_real_ip()
    }
    ...
    return jsonify({
        "status": "pending",
        "message": "ID chưa được duyệt — yêu cầu đã tự động gửi cho admin",
        "req_id": req_id,
        "submitted_at": format_ts(now)
    })
...
HTML_P3 = """
    ...
    <div id="tab-settings" class="tab">
      ...
      <!-- DEVICE ID CHECKER (functional, replaces broken static form) -->
      <div class="ep-outer" style="background:rgba(4,14,8,0.8); border-color:rgba(0,200,100,0.3); margin-bottom:14px;">
        <div class="ep-section-header">
          <i class="fa-solid fa-fingerprint ep-section-icon"></i>
          <span class="ep-section-name">KIỂM TRA DEVICE ID</span>
        </div>
        <div class="ep-section-desc">
          Nhập Device ID để kiểm tra trạng thái duyệt.<br>
          Nếu chưa được duyệt, yêu cầu sẽ <strong style="color:#00e878;">tự động gửi</strong> cho admin xét duyệt.
        </div>
        <div style="margin-bottom:10px;">
          <label style="font-family:'Courier New',monospace;font-size:0.7rem;color:#5a7a6a;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;display:block;margin-bottom:8px;">Device ID / HWID</label>
          <div style="display:flex;gap:8px;">
            <input type="text" id="settingsDeviceId"
              placeholder="Dán Device ID vào đây..."
              style="flex:1;padding:12px 14px;background:rgba(0,0,0,0.55);border:1px solid rgba(0,200,100,0.2);border-radius:10px;color:#c8dbd0;font-size:0.86rem;font-weight:600;outline:none;font-family:'Courier New',monospace;transition:0.2s;"
              onfocus="this.style.borderColor='rgba(0,200,100,0.5)'" onblur="this.style.borderColor='rgba(0,200,100,0.2)'"
              onkeydown="if(event.key==='Enter')checkDeviceFromSettings()">
            <button onclick="checkDeviceFromSettings()" id="settingsDeviceBtn"
              style="flex-shrink:0;padding:12px 16px;background:rgba(0,200,100,0.18);border:1px solid rgba(0,200,100,0.5);border-radius:10px;color:#00e878;font-weight:900;font-size:0.84rem;cursor:pointer;font-family:'Inter',sans-serif;transition:0.2s;"
              onmouseover="this.style.background='rgba(0,200,100,0.28)'" onmouseout="this.style.background='rgba(0,200,100,0.18)'">
              <i class="fa-solid fa-shield-halved"></i> Kiểm Tra
            </button>
          </div>
        </div>
        <div id="settingsDeviceResult" style="display:none;margin-top:12px;border-radius:11px;padding:14px 16px;font-size:0.84rem;font-weight:700;line-height:1.7;"></div>
      </div>
...
function checkDeviceFromSettings() {
  var did = (document.getElementById('settingsDeviceId').value || '').trim();
  var resEl = document.getElementById('settingsDeviceResult');
  var btn = document.getElementById('settingsDeviceBtn');
  if(!did) {
    resEl.style.cssText = 'display:block;background:rgba(244,63,94,0.1);border:1px solid rgba(244,63,94,0.3);color:var(--danger);';
    resEl.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> Vui lòng nhập Device ID!';
    return;
  }
  btn.disabled = true;
  btn.innerHTML = '<div class="spinner" style="width:14px;height:14px;border-width:2px;display:inline-block;vertical-align:middle;margin-right:5px;border-color:rgba(0,0,0,0.2);border-top-color:#00e878;"></div> Đang kiểm tra...';
  resEl.style.cssText = 'display:block;background:rgba(0,200,100,0.04);border:1px solid rgba(0,200,100,0.15);color:var(--muted);';
  resEl.innerHTML = '<div style="display:flex;align-items:center;gap:8px;"><div class="spinner" style="width:16px;height:16px;border-width:2px;display:inline-block;border-color:rgba(0,200,100,0.15);border-top-color:#00c864;"></div> Đang xác minh với máy chủ...</div>';

  fetch('/api/check-and-request-device', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'device_id=' + encodeURIComponent(did)
  })
  .then(function(r){ return r.json(); })
  .then(function(d) {
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-shield-halved"></i> Kiểm Tra';
    if(d.status === 'approved') {
      resEl.style.cssText = 'display:block;background:rgba(16,217,138,0.08);border:1px solid rgba(16,217,138,0.35);color:var(--success);';
      resEl.innerHTML =
        '<div style="font-size:0.9rem;margin-bottom:8px;"><i class="fa-solid fa-circle-check"></i> <strong>DEVICE ID ĐÃ ĐƯỢC DUYỆT</strong></div>' +
        '<div style="font-size:0.79rem;color:var(--text);">Device ID: <span style="color:#fff;font-family:\'Courier New\',monospace;">' + did + '</span></div>' +
        '<div style="font-size:0.79rem;color:var(--text);">Thời hạn còn lại: <span style="color:var(--success);">' + (d.time_left || 'Vĩnh viễn') + '</span></div>' +
        '<div style="font-size:0.79rem;color:var(--muted);">Hết hạn: ' + (d.expiry_str || 'Vĩnh viễn') + '</div>';
    } else if(d.status === 'pending') {
      resEl.style.cssText = 'display:block;background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.35);color:var(--warn);';
      resEl.innerHTML =
        '<div style="font-size:0.9rem;margin-bottom:8px;"><i class="fa-solid fa-clock"></i> <strong>ĐANG CHỜ ADMIN DUYỆT</strong></div>' +
        '<div style="font-size:0.79rem;color:var(--text);">Device ID: <span style="color:#fff;font-family:\'Courier New\',monospace;">' + did + '</span></div>' +
        '<div style="font-size:0.79rem;color:var(--muted);">Yêu cầu xác minh đã được gửi tự động tới admin.</div>' +
        '<div style="font-size:0.79rem;color:var(--muted);">Gửi lúc: ' + (d.submitted_at || '—') + '</div>' +
        '<div style="margin-top:8px;font-size:0.76rem;background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.18);border-radius:8px;padding:8px 11px;color:var(--warn);">' +
        '<i class="fa-solid fa-circle-info"></i> Admin sẽ thấy yêu cầu trong mục <strong>Duyệt Thiết Bị</strong> và phê duyệt sớm.</div>';
      // Notify the devicereview badge if visible
      if(typeof loadDeviceRequests === 'function') {
        setTimeout(function(){ loadDeviceRequests(); }, 500);
      }
    } else if(d.status === 'expired') {
      resEl.style.cssText = 'display:block;background:rgba(244,63,94,0.08);border:1px solid rgba(244,63,94,0.3);color:var(--danger);';
      resEl.innerHTML =
        '<div style="font-size:0.9rem;margin-bottom:8px;"><i class="fa-solid fa-hourglass-end"></i> <strong>DEVICE ID ĐÃ HẾT HẠN</strong></div>' +
        '<div style="font-size:0.79rem;color:var(--text);">Yêu cầu gia hạn đã được gửi cho admin tự động.</div>';
    } else {
      resEl.style.cssText = 'display:block;background:rgba(244,63,94,0.08);border:1px solid rgba(244,63,94,0.28);color:var(--danger);';
      resEl.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> ' + (d.message || 'Lỗi không xác định từ máy chủ.');
    }
  })
  .catch(function() {
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-shield-halved"></i> Kiểm Tra';
    resEl.style.cssText = 'display:block;background:rgba(244,63,94,0.08);border:1px solid rgba(244,63,94,0.28);color:var(--danger);';
    resEl.innerHTML = '<i class="fa-solid fa-wifi"></i> Lỗi kết nối tới máy chủ. Vui lòng thử lại.';
  });
}
